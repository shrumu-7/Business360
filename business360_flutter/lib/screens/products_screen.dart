...
class _ProductsScreenState extends State<ProductsScreen> {
  ...
  Future<void> addOrEdit([Product? old]) async {
    ...
    product.category = category.text.trim();
    product.price = double.tryParse(price.text) ?? 0;
    product.cost = double.tryParse(cost.text) ?? 0;
    product.stock = double.tryParse(stock.text) ?? 0;
    product.unit = unit.text.trim().isEmpty ? 'pcs' : unit.text.trim();
  ...
  List<_ImportedProduct> _parseOcr(String text) {
    ...
    for (final rawLine in text.split(RegExp(r'\r?\n'))) {
      ...
      final values = matches.map((m) => double.tryParse(m.group(0)!.replaceAll(',', '.'))).whereType<double>().toList();
      if (values.length < 3) continue;
      final first = matches.first;
      final name = line.substring(0, first.start).replaceAll(RegExp(r'[-|:]'), '').trim();
  ...
  Future<void> _showOcrHelp() async {
    await showDialog<void>(
      context: context,
      builder: (dialogContext) => AlertDialog(
        title: const Text('No product rows detected'),
        content: const Text('No readable product rows were found. Please use a clear screenshot/photo where each row contains product name, purchase price, selling price and stock.'),
          ...
          FilledButton(
            onPressed: () async {
              ...
              for (final c in controllers) {
                final productName = c[0].text.trim();
  ...
  void _disposeControllers(List<List<TextEditingController>> controllers) {
    for (final list in controllers) {
      for (final controller in list) controller.dispose();
    }
  }
  ...
  Future<void> _saveImportedProducts(List<_ImportedProduct> imported) async {
    ...
    for (final row in imported) {
      ...
      for (final product in current) {
        if (_normalize(product.name) == key) {
          existing = product;
          break;
        }
      }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            tooltip: 'Smart Stock Import',
            onPressed: smartStockImport,
                      ...
                      child: ListTile(
                        ...
                        subtitle: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              if (p.category.isNotEmpty) Text('Category: ' + p.category),
                              Text('Purchase: ' + currency + p.cost.toStringAsFixed(2)),
                              Text('Selling: ' + currency + p.price.toStringAsFixed(2)),
                              Text(
                                'Current stock: ' + p.stock.toString() + ' ' + p.unit,
                                style: TextStyle(
                                  fontWeight: FontWeight.w600,
                                  color: low ? Theme.of(context).colorScheme.error : null,
                                ),
                              ),
                              if (low) const Text('LOW STOCK'),
                            ],
                          ),
                        ),
                        trailing: Wrap(children: [
                          IconButton(onPressed: () => addOrEdit(p), icon: const Icon(Icons.edit)),
                          IconButton(onPressed: () => remove(p), icon: const Icon(Icons.delete_outline)),
                        ]),
                      ),
                    );
                  },
