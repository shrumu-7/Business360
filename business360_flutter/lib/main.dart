...
class _Business360AppState extends State<Business360App> {
  ...
  @override
  void initState() { super.initState(); store = BusinessStore(widget.prefs); _load(); }
  void _load() {
    ...
    locked = widget.prefs.getString('securityMode') != null && widget.prefs.getString('securityMode') != 'none' &&
        ((widget.prefs.getString('pinHash') ?? '').isNotEmpty || (widget.prefs.getString('patternHash') ?? '').isNotEmpty);
  }
  ...
  Color background() {
    switch (appBg) {
      ...
      case 'blueWhite': return const Color(0xFFF4F7FC);
      default: return const Color(0xFFF4F7FC);
    }
  }
  Color seedColor() {
    const colors = <String, Color>{
      'purple': Color(0xFF6C57F5), 'blue': Color(0xFF087EA4), 'green': Color(0xFF15803D), 'rose': Color(0xFFE11D48),
      'orange': Color(0xFFEA580C), 'dark': Color(0xFF686FF0), 'sky': Color(0xFF0284C7), 'cream': Color(0xFFD97706),
      'mint': Color(0xFF059669), 'premium': Color(0xFFB08D57), 'light': Color(0xFF64748B), 'ocean': Color(0xFF0369A1),
      'sunset': Color(0xFFF97316), 'aurora': Color(0xFF7C3AED), 'forest': Color(0xFF166534), 'gold': Color(0xFFCA8A04),
    };
  ...
  Widget build(BuildContext context) => MaterialApp(
    debugShowCheckedModeBanner: false, title: 'Business360', theme: appTheme(Brightness.light), darkTheme: appTheme(Brightness.dark), themeMode: mode,
    home: locked ? LockScreen(prefs: widget.prefs, onUnlocked: () => setState(() => locked = false)) : HomeScreen(prefs: widget.prefs, store: store, dark: mode == ThemeMode.dark, onTheme: toggleTheme, onSettingsChanged: refresh),
  );
}

class LockScreen extends StatefulWidget {
  const LockScreen({super.key, required this.prefs, required this.onUnlocked});
  final SharedPreferences prefs; final VoidCallback onUnlocked;
...
class _LockScreenState extends State<LockScreen> {
  ...
  void unlock() {
    ...
    if (expected != null && expected.isNotEmpty && hash(input.text) == expected) widget.onUnlocked();
    else { setState(() => error = 'ভুল কোড। আবার চেষ্টা করুন।'); input.clear(); }
  }
  @override Widget build(BuildContext context) {
    return Scaffold(body: Center(child: ConstrainedBox(
      constraints: const BoxConstraints(maxWidth: 380),
      child: Card(margin: const EdgeInsets.all(24), child: Padding(
...
class DashboardPage extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    final products = store.products, customers = store.customers, sales = store.sales, currency = prefs.getString('currency') ?? '৳';
