...
class _Business360AppState extends State<Business360App> {
  ...
  @override Widget build(BuildContext context) => MaterialApp(
    debugShowCheckedModeBanner: false, title: 'Business360', themeMode: mode,
    theme: ThemeData(useMaterial3: true, colorSchemeSeed: const Color(0xFF6C57F5), scaffoldBackgroundColor: const Color(0xFFF6F7FB)),
    darkTheme: ThemeData(useMaterial3: true, colorSchemeSeed: const Color(0xFF8B7CFF), brightness: Brightness.dark),
    home: HomeScreen(index: index, onIndexChanged: (v) => setState(() => index = v), onTheme: toggleTheme, dark: mode == ThemeMode.dark, store: store),
  );
}

class HomeScreen extends StatelessWidget {
  const HomeScreen({super.key, required this.index, required this.onIndexChanged, required this.onTheme, required this.dark, required this.store});
  final int index; final ValueChanged<int> onIndexChanged; final VoidCallback onTheme; final bool dark; final BusinessStore store;
  ...
  @override Widget build(BuildContext context) => Scaffold(
    ...
    drawer: Drawer(child: SafeArea(child: ListView(padding: const EdgeInsets.all(12), children: [
      ...
      for (var i = 0; i < pages.length; i++) ListTile(
        leading: Icon([Icons.dashboard, Icons.point_of_sale, Icons.inventory_2, Icons.people, Icons.bar_chart][i]), title: Text(pages[i]), selected: index == i,
...
class DashboardPage extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    ...
    final totalSales = sales.fold<double>(0, (a, s) => a + s.total);
    final profit = sales.fold<double>(0, (a, s) => a + s.profit);
    final due = customers.fold<double>(0, (a, c) => a + c.due);
    final stockValue = products.fold<double>(0, (a, p) => a + p.cost * p.stock);
    final cards = [
      ('Sales', '৳${totalSales.toStringAsFixed(2)}', Icons.point_of_sale),
      ('Profit', '৳${profit.toStringAsFixed(2)}', Icons.trending_up),
      ('Products', '${products.length}', Icons.inventory_2),
      ('Customers', '${customers.length}', Icons.people),
      ('Due', '৳${due.toStringAsFixed(2)}', Icons.account_balance_wallet),
      ('Stock value', '৳${stockValue.toStringAsFixed(2)}', Icons.warehouse),
    ];
    return ListView(padding: const EdgeInsets.all(16), children: [
      Card(child: Padding(padding: const EdgeInsets.all(20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: const [
        Text('Business360', style: TextStyle(fontSize: 26, fontWeight: FontWeight.bold)), SizedBox(height: 6), Text('Smart Business Management'),
      ]))),
      const SizedBox(height: 12),
      GridView.builder(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), itemCount: cards.length,
        ...
        itemBuilder: (_, i) => Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
          Icon(cards[i].$3, size: 30), const SizedBox(height: 10), Text(cards[i].$1), const SizedBox(height: 4), Text(cards[i].$2, style: const TextStyle(fontSize: 19, fontWeight: FontWeight.bold)),
        ]))),
      ),
    ]);
  }
