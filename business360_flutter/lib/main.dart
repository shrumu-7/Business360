  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: HomeScreen(
        ...
        onIndexChanged: (v) => setState(() => index = v),
        onTheme: toggleTheme,
...
class HomeScreen extends StatelessWidget {
  ...
  void openSettings(BuildContext context) {
    showModalBottomSheet(
      ...
      builder: (_) => FractionallySizedBox(
        heightFactor: .94,
        child: SettingsScreen(prefs: prefs, store: store, onChanged: onSettingsChanged),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(index < pages.length ? pages[index] : shop),
        actions: [
          IconButton(onPressed: onTheme, icon: Icon(dark ? Icons.light_mode : Icons.dark_mode)),
          IconButton(onPressed: () => openSettings(context), icon: const Icon(Icons.settings)),
        ],
      ),
      drawer: Drawer(
        child: SafeArea(
          child: ListView(
            ...
            children: [
              Padding(
                ...
                child: Text(shop, style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
              ),
              for (var i = 0; i < pages.length; i++)
                ListTile(
                  ...
                  title: Text(pages[i]),
                  selected: index == i,
                  onTap: () { Navigator.pop(context); onIndexChanged(i); },
                ),
              const Divider(),
              ListTile(leading: const Icon(Icons.settings), title: const Text('Settings'), onTap: () { Navigator.pop(context); openSettings(context); }),
            ],
          ),
        ),
      ),
...
class DashboardPage extends StatelessWidget {
  ...
  Color dashboardBackground() {
    switch (prefs.getString('dashboardBg') ?? 'midnight') {
      case 'blue': return const Color(0xFF0B3B60);
      case 'plain': return Colors.transparent;
      default: return const Color(0xFF151B2B);
    }
  }

  Color cardColor(BuildContext context, int i) {
    const colors = [Color(0xFF157F5B), Color(0xFF2D5BBA), Color(0xFF7A4FB2), Color(0xFFB94A48), Color(0xFF176F8B), Color(0xFF8B6A22), Color(0xFF7A3F88), Color(0xFF336B70)];
  ...
  Widget build(BuildContext context) {
    ...
    final totalSales = sales.fold<double>(0, (s, x) => s + x.total);
    final profit = sales.fold<double>(0, (s, x) => s + x.profit);
    final due = customers.fold<double>(0, (s, x) => s + x.due);
    final stock = products.fold<double>(0, (s, x) => s + x.cost * x.stock);
    final expenses = store.expenses.fold<double>(0, (s, x) => s + x.amount);
    final purchases = store.purchases.fold<double>(0, (s, x) => s + x.amount);
    final cards = [
      ('Sales', '$currency${totalSales.toStringAsFixed(2)}', Icons.point_of_sale),
      ('Profit', '$currency${profit.toStringAsFixed(2)}', Icons.trending_up),
      ('Products', '${products.length}', Icons.inventory_2),
      ('Customers', '${customers.length}', Icons.people),
      ('Due', '$currency${due.toStringAsFixed(2)}', Icons.account_balance_wallet),
      ('Stock value', '$currency${stock.toStringAsFixed(2)}', Icons.warehouse),
      ('Expenses', '$currency${expenses.toStringAsFixed(2)}', Icons.money_off),
      ('Purchases', '$currency${purchases.toStringAsFixed(2)}', Icons.shopping_cart),
    ];
    final dashboardBg = dashboardBackground();
    final cardStyle = prefs.getString('dashboardCardBg') ?? 'gradient';
    final border = prefs.getString('dashboardBorder') ?? 'glow';
    return Container(
      color: dashboardBg,
      child: ListView(
        padding: const EdgeInsets.all(16),
        children: [
          Card(
            color: cardStyle == 'glass' ? Colors.white.withValues(alpha: .12) : null,
            child: Padding(
              padding: const EdgeInsets.all(20),
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(prefs.getString('shop') ?? 'আমার দোকান', style: const TextStyle(fontSize: 26, fontWeight: FontWeight.bold)),
                const SizedBox(height: 6),
                Text(prefs.getString('address') ?? 'Smart Business Management'),
                if ((prefs.getString('phone') ?? '').isNotEmpty) Text(prefs.getString('phone')!),
              ]),
            ),
          ),
          ...
          GridView.builder(
            ...
            itemBuilder: (context, i) {
              final c = cards[i];
              final bg = cardStyle == 'solid' ? cardColor(context, i) : cardStyle == 'glass' ? Colors.white.withValues(alpha: .14) : cardColor(context, i);
              return Card(
                color: bg,
                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16), side: border == 'none' ? BorderSide.none : BorderSide(color: border == 'glow' ? Colors.white.withValues(alpha: .35) : Colors.white.withValues(alpha: .15), width: border == 'glow' ? 1.5 : 1)),
                child: Padding(
                  padding: const EdgeInsets.all(16),
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
                    ...
                    Text(c.$2, style: const TextStyle(fontSize: 19, fontWeight: FontWeight.bold)),
                  ]),
                ),
              );
            },
